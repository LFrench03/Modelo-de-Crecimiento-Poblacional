...
\textbf{1-Cálculo de la matriz $\omega$:}\\
$\omega$ es una matriz que contiene las derivadas parciales de la función de pérdida con respecto a los parámetros K y r. Se calcula como: 
...
\small{$$\omega = \frac{\delta(r_{min})}{\delta(K)} = \sum_{i=0}^{N}[2(P(t_{i}) - f(tdata, K, r)) \cdot (\frac{-f(tdata, K, r)}{K^{2}})]$$}

\textbf{Y sustituyendo  f(tdata, K, r) en la derivada, obtenemos:}
\small{$$\omega = \frac{\delta(r_{min})}{\delta(K)} =$$} 
...
\small{Evaluando estas derivadas en los valores iniciales de K y r, obtenemos $\omega$}
...
\end{figure}
\\
Luego, para una prediccion utilizando las mismas estimaciones para años posteriores (2020-2030) se obtiene:
